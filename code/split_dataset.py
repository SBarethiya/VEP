...
def load_dataset_from_tsv(path):
    import csv
...
def train_tune_test(ds, train_size=.90, tune_size=.1, test_size=0., train_n=None, withhold=None,
    ...
    if train_size > 0:
        ...
        if adjusted_train_size == 1:
            ...
            if train_n:
                split["train"] = train_idxs[:train_n]
            else:
                split["train"] = train_idxs
                train_n=len(train_idxs)
    ...
    if out_dir is not None:
        ...
        out_dir_split = join(out_dir, "tr_n{}_tr{}_te{}_r{}".format(train_n, train_size, test_size, rseed))
...
def position_split(ds, seq_len, wt_ofs, train_pos_size, tune_size, rseed=8, out_dir=None, overwrite=False):
    ...
    if tune_size > 0:
        if tune_size == 1:
            ...
            test_idxs, tune_idxs = train_test_split(test_pool_idxs, test_size=tune_size)
            split["test"] = test_idxs
            split["tune"] = tune_idxs
    else:
